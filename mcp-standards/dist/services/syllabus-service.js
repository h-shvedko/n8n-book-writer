...
exports.syllabusService = exports.SyllabusService = void 0;
const syllabus_1 = require("../types/syllabus");
class SyllabusService {
     ...
     */
    loadSyllabus(data) {
        const parsed = syllabus_1.SyllabusSchema.parse(data);
     ...
     */
    searchTopics(domain, topics, path, searchId, results) {
        for (const topic of topics) {
            ...
            if (topic.subtopics && topic.subtopics.length > 0) {
                this.searchTopics(domain, topic.subtopics, currentPath, searchId, results);
            }
        }
    }
    /**
    ...
    searchTopicsByKeyword(domain, topics, path, searchTerm, results) {
        for (const topic of topics) {
            const currentPath = [...path, topic.title];
            ...
            if (topic.subtopics) {
                this.searchTopicsByKeyword(domain, topic.subtopics, currentPath, searchTerm, results);
            }
        }
    }
    /**
     ...
     */
    updateSection(domainId, updatedDomain) {
        if (!this.syllabus) {
            throw new Error('Syllabus not loaded');
        }
    ...
    topicsToMarkdown(topics, level) {
        ...
        for (const topic of topics) {
            ...
            if (topic.subtopics && topic.subtopics.length > 0) {
                md += this.topicsToMarkdown(topic.subtopics, level + 1);
            }
        }
        return md;
    }
...
// Singleton instance
exports.syllabusService = new SyllabusService();
