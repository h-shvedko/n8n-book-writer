...
export const FilterMetadataSchema = z.object({
  ...
  document_type: z.string().optional().describe('Filter by document type (e.g., "pdf", "docx", "markdown")'),
  domain_id: z.string().optional().describe('Filter by syllabus domain ID'),
  topic_id: z.string().optional().describe('Filter by syllabus topic ID'),
  tags: z.array(z.string()).optional().describe('Filter by tags'),
});

// Metadata schema for ingestion
export const IngestMetadataSchema = z.object({
  source: z.string().optional().describe('Document source identifier'),
  title: z.string().optional().describe('Document title'),
  author: z.string().optional().describe('Document author'),
  document_type: z.string().optional().describe('Document type'),
  domain_id: z.string().optional().describe('Related syllabus domain ID'),
  topic_id: z.string().optional().describe('Related syllabus topic ID'),
  tags: z.array(z.string()).optional().describe('Document tags'),
  language: z.string().default('en').describe('Document language'),
});

// Tool Input Schemas
export const HybridSearchInputSchema = z.object({
  query: z.string().min(1).describe('Search query text'),
  filter_metadata: FilterMetadataSchema.optional().describe('Optional filter conditions'),
  limit: z.number().min(1).max(100).default(10).describe('Maximum number of results to return'),
  vector_weight: z.number().min(0).max(1).default(0.7).describe('Weight for vector similarity (0-1)'),
  keyword_weight: z.number().min(0).max(1).default(0.3).describe('Weight for keyword matching (0-1)'),
  output_format: z.enum(['json', 'markdown']).default('json').describe('Output format'),
});

export const IngestAndEmbedInputSchema = z.object({
  text: z.string().min(1).describe('Text content to ingest and embed'),
  metadata: IngestMetadataSchema.describe('Metadata for the document'),
  output_format: z.enum(['json', 'markdown']).default('json').describe('Output format'),
});
...
export const DeleteDocumentsInputSchema = z.object({
  filter_metadata: FilterMetadataSchema.describe('Filter conditions for documents to delete'),
});
